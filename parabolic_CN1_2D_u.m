function p = parabolic_CN1_2D_u(u, Qu, dt)
...
for j = 2:N+1 % cell-centered interior
    for i = 2:M % node-based interior
        d(i, j) = d2*u(i, j+1)+(1-2*d2)*u(i, j)+d2*u(i, j-1)+Qu(i, j)*dt/2;
    end
end
...
u(2:M, 2:N+1) = reshape(mySolveTriDiag(a, b, c, d), [M-1, N]);
% reshape from 1D to 2D and store a, b, c, d
tempa = reshape(a, [M-1, N]); tempb = reshape(b, [M-1, N]);
tempc = reshape(c, [M-1, N]); tempd = reshape(d, [M-1, N]);
